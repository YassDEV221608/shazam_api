...
import { getTopCharts,getTopArtists, searchTrack, serachArtists } from '../utils/fetchFromAPI';
...
const Search = () => {
    ...
    useEffect(() => {
        const query = extractQueryFromURL(location.pathname);
        Promise.all([searchTrack(query), serachArtists(query)])
